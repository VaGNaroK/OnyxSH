...
class CommTerminalApp(Adw.Application):
    ...
    def _initialize_subsystems(self) -> bool:
        ...
        try:
            ...
            handle_exception(
                e, "application initialization", "onyxsh.app", reraise=True
            )
            return False
    ...
    def _configure_icon_theme(self) -> None:
        ...
        When 'onyxsh' strategy is selected, icons are loaded directly from
        ...
        try:
            ...
            icons._use_bundled_icons = icon_strategy == "onyxsh"

            self.logger.info(
                f"Icon strategy: {'OnyxSH (bundled SVGs)' if icon_strategy == 'onyxsh' else 'System icons'}"
            )
    ...
    def _setup_keyboard_shortcuts(self) -> None:
        ...
        try:
            ...
            self.set_accels_for_action("app.preferences", ["F2"])
            self.set_accels_for_action("win.preferences", ["F2"])
    ...
    def _show_update_available_dialog(
        ...
        dialog = Adw.MessageDialog(
            ...
            body=_(
                "A new OnyxSH version is available.\n\nCurrent: {local}\nLatest: {remote}\n\nDo you want to update now?"
    ...
    def _show_update_restart_dialog(self, remote_version: str) -> None:
        ...
        dialog = Adw.MessageDialog(
            ...
            body=_(
                "OnyxSH was updated to version {version}. You need to restart the application to apply all changes."
    ...
    def _restart_application(self) -> None:
        ...
        try:
            launcher = shutil.which("onyxsh")
            if launcher:
                subprocess.Popen([launcher], start_new_session=True)
            else:
                subprocess.Popen(
                    [sys.executable, "-m", "onyxsh"],
            ...
            self._show_error_dialog(
                _("Restart Failed"),
                _("Could not restart automatically. Please close and open OnyxSH again."),
            )
    ...
    def _normalize_working_directory_arg(
        ...
        if path.startswith("/app") or path.startswith("/usr") or path.startswith("/opt/onyxsh") or path == "/":
            return None
        return path
    ...
    def _on_backup_now_action(self, _action, _param) -> None:
        ...
        file_dialog.set_initial_name(f"onyxsh-backup-{timestamp}.7z")
    ...
    def _show_restore_success_dialog(self):
        dialog = Adw.MessageDialog(
            ...
            body=_(
                "Data has been restored successfully. Please restart OnyxSH for the changes to take effect."
            ),
            close_response="ok",
        )
