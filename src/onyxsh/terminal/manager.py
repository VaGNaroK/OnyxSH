...
class ManualSSHTracker:
    def __init__(self, registry, on_state_changed_callback):
        self.logger = get_logger("onyxsh.terminal.ssh_tracker")
        self.registry = registry
...
class TerminalRegistry:
    def __init__(self):
        self.logger = get_logger("onyxsh.terminal.registry")
...
class TerminalManager:
    def __init__(self, parent_window, settings_manager: SettingsManager):
        self.logger = get_logger("onyxsh.terminal.manager")
    ...
    def _setup_terminal_events(
        ...
        try:
            ...
            terminal.onyxsh_controllers.append(click_controller)
            ...
            terminal.onyxsh_controllers.append(right_click_controller)
            ...
            terminal.onyxsh_controllers.append(key_controller)

            # Modern Gtk.ShortcutController directly on the terminal widget
            term_shortcut_ctrl = Gtk.ShortcutController.new()
            term_shortcut_ctrl.set_scope(Gtk.ShortcutScope.MANAGED)
            term_shortcut_ctrl.add_shortcut(
                Gtk.Shortcut.new(
    ...
    def _show_balabit_gateway_auth_dialog(
        ...
        dialog.add_css_class("onyxsh-dialog")
    ...
    def _cleanup_terminal(self, terminal: Vte.Terminal, terminal_id: int) -> None:
        ...
        with self._cleanup_lock:
            ...
            if hasattr(terminal, "onyxsh_handler_ids"):
                for handler_id in terminal.onyxsh_handler_ids:
                    if GObject.signal_handler_is_connected(terminal, handler_id):
                        terminal.disconnect(handler_id)
                terminal.onyxsh_handler_ids.clear()

            if hasattr(terminal, "onyxsh_controllers"):
                for controller in terminal.onyxsh_controllers:
                    terminal.remove_controller(controller)
                terminal.onyxsh_controllers.clear()
    ...
    def _setup_url_patterns(self, terminal: Vte.Terminal) -> None:
        try:
            ...
            if hasattr(terminal, "connect"):
                ...
                if not hasattr(terminal, "onyxsh_handler_ids"):
                    terminal.onyxsh_handler_ids = []
                terminal.onyxsh_handler_ids.append(handler_id)
