# onyxsh/terminal/_highlighter_impl.py
"""
...
class HighlightedTerminalProxy:
    ...
    def __init__(
        ...
        """
        self.logger = get_logger("onyxsh.terminal.proxy")
    ...
    def _handle_semantic_stream_bytes(self, data: bytes) -> None:
        ...
        try:
            ...
            for match in re.finditer(
                ...
                self.logger.debug(
                    f"PTY stream detected OSC 133: action={action}, param='{param}'"
                )
            ...
            if not osc133_found:
                for match in re.finditer(
                    ...
                    self.logger.debug(
                        f"PTY stream detected __zt_sem__: action={action}, param='{param}'"
                    )
                    tracker.handle_osc133(term, action, param)
        except Exception as e:
            self.logger.debug(f"PTY stream parsing error: {e}")
