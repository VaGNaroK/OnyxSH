...
class ProcessTracker:
    ...
    def __init__(self):
        self.logger = get_logger("onyxsh.spawner.tracker")
...
class ProcessSpawner:
    ...
    def __init__(self):
        self.logger = get_logger("onyxsh.spawner")
    ...
    def _prepare_shell_environment(
        ...
        osc7_command = (
            f"__onyxsh_osc7() {{ {OSC7_HOST_DETECTION_SNIPPET} "
            'printf "\\033]7;file://%s%s\\007" "$ONYXSH_OSC7_HOST" "$PWD"; }; __onyxsh_osc7'
        )

        # Prepare shared shell init directory in user cache (shared between Flatpak sandbox and Host)
        cache_dir = Path.home() / ".cache" / "onyxsh" / "shell_init"
        ...
        if shell_basename == "zsh":
            try:
                ...
                zshrc_content = (
                    f"_onyxsh_update_cwd() {{ {osc7_command}; }}\n"
                    '__onyxsh_semantic_precmd() {\n'
                    '  local exit_code=$?\n'
                    '  local last_cmd=$(fc -ln -1 2>/dev/null | sed -e "s/^[[:space:]]*//")\n'
                    '  local b64_cmd=$(printf "%s" "$last_cmd" | base64 2>/dev/null | tr -d "\\r\\n")\n'
                    '  printf "\\033]133;D;%d\\007\\033]133;A\\007\\033]7;file://%s%s?__zt_sem__=D_%d_%s__A\\007\\033]6;file://localhost/__zt_sem__/D_%d_%s__A\\007\\033]0;__zt_sem__:D:%d;A\\007" "$exit_code" "$ONYXSH_OSC7_HOST" "$PWD" "$exit_code" "$b64_cmd" "$exit_code" "$b64_cmd" "$exit_code"\n'
                    '}\n'
                    '__onyxsh_semantic_preexec() {\n'
                    '  local b64_cmd=$(printf "%s" "$1" | base64 2>/dev/null | tr -d "\\r\\n")\n'
                    '  printf "\\033]133;C\\007\\033]7;file://%s%s?__zt_sem__=C_%s\\007\\033]6;file://localhost/__zt_sem__/C\\007\\033]0;__zt_sem__:C\\007" "$ONYXSH_OSC7_HOST" "$PWD" "$b64_cmd"\n'
                    '}\n'
                    'if [[ -z "$precmd_functions" ]]; then typeset -a precmd_functions; fi\n'
                    'precmd_functions+=(__onyxsh_semantic_precmd)\n'
                    'if [[ -z "$preexec_functions" ]]; then typeset -a preexec_functions; fi\n'
                    'preexec_functions+=(__onyxsh_semantic_preexec)\n'
            ...
            try:
                bash_init_path = str(cache_dir / "onyxsh_bashrc")
                login_bootstrap = ""
                if use_login_shell:
                    login_bootstrap = (
                ...
                bashrc_content = (
                    ...
                    +
                    '__onyxsh_semantic_preexec() {\n'
                    '  if [ -n "$COMP_LINE" ] || [ -n "$BASH_COMPLETION" ]; then return; fi\n'
                    '  case "$BASH_COMMAND" in\n'
                    '    *__onyxsh_*|*_onyxsh_*|*PROMPT_COMMAND*) return ;;\n'
                    '  esac\n'
                    '  if [ "${__onyxsh_preexec_done:-0}" -eq 0 ]; then\n'
                    '    __onyxsh_preexec_done=1\n'
                    '    local b64_cmd=$(printf "%s" "$BASH_COMMAND" | base64 2>/dev/null | tr -d "\\r\\n")\n'
                    '    printf "\\033]133;C\\007\\033]7;file://%s%s?__zt_sem__=C_%s\\007\\033]6;file://localhost/__zt_sem__/C\\007\\033]0;__zt_sem__:C\\007" "$ONYXSH_OSC7_HOST" "$PWD" "$b64_cmd"\n'
                    '  fi\n'
                    '}\n'
                    '__onyxsh_semantic_precmd() {\n'
                    '  local exit_code=$?\n'
                    '  local last_cmd=$(HISTTIMEFORMAT= history 1 2>/dev/null | sed -e "s/^[[:space:]]*[0-9]*[[:space:]]*//")\n'
                    '  local b64_cmd=$(printf "%s" "$last_cmd" | base64 2>/dev/null | tr -d "\\r\\n")\n'
                    '  printf "\\033]133;D;%d\\007\\033]133;A\\007\\033]7;file://%s%s?__zt_sem__=D_%d_%s__A\\007\\033]6;file://localhost/__zt_sem__/D_%d_%s__A\\007\\033]0;__zt_sem__:D:%d;A\\007" "$exit_code" "$ONYXSH_OSC7_HOST" "$PWD" "$exit_code" "$b64_cmd" "$exit_code" "$b64_cmd" "$exit_code"\n'
                    '  __onyxsh_preexec_done=0\n'
                    '}\n'
                    'trap "__onyxsh_semantic_preexec" DEBUG\n'
                    'if [ -z "$PROMPT_COMMAND" ]; then\n'
                    '  PROMPT_COMMAND="__onyxsh_semantic_precmd"\n'
                    ...
                    '  case ";${PROMPT_COMMAND};" in\n'
                    '    *";__onyxsh_semantic_precmd;"*) ;;\n'
                    '    *) PROMPT_COMMAND="${PROMPT_COMMAND};__onyxsh_semantic_precmd" ;;\n'
                    '  esac\n'
                    'fi\n'
                    '__onyxsh_semantic_precmd\n'
                )
                ...
                env["ONYXSH_BASH_INIT"] = bash_init_path
                self.logger.info(
        ...
        if in_flatpak:
            ...
            if os.path.exists(host_spawn_bin):
                ...
                if shell_basename == "bash" and "ONYXSH_BASH_INIT" in env:
                    cmd.extend([shell, "--rcfile", env["ONYXSH_BASH_INIT"], "-i"])
        ...
        if use_login_shell:
            if shell_basename == "bash" and "ONYXSH_BASH_INIT" in env:
                cmd = [shell, "--rcfile", env["ONYXSH_BASH_INIT"], "-i"]
            else:
                cmd = [shell, "-l"]
        else:
            if shell_basename == "bash" and "ONYXSH_BASH_INIT" in env:
    ...
    def execute_remote_command_sync(
        ...
        try:
            ...
            log_error_with_context(
                e, f"Remote command execution for {session.name}", "onyxsh.spawner"
            )
            return False, str(e)
    ...
    def _build_remote_command_secure(
        ...
        if command_type == "ssh" and getattr(
            ...
            osc7_prompt_command = (
                ...
                'printf "\\033]7;file://%s%s\\007" "$ONYXSH_OSC7_HOST" "$PWD"'
            )
            shell_bootstrap = (
    ...
    def _resolve_and_validate_working_directory(
        ...
        try:
            ...
            if resolved_path.startswith("/app") or resolved_path.startswith("/usr") or resolved_path.startswith("/opt/onyxsh") or resolved_path == "/":
                return str(self.platform_info.home_dir)
...
class SSHConnectionChecker:
    ...
    def __init__(self, spawner: Optional[ProcessSpawner] = None):
        ...
        """
        self.logger = get_logger("onyxsh.ssh_checker")
