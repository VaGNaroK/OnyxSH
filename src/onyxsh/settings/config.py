...
class AppConstants:
    ...
    DEVELOPER_TEAM = [
        "VaGNaroK (Maintainer)",
        "Leonardo Berbert (Original Author of Zashterminal)",
    ]
    COPYRIGHT = "© 2026 VaGNaroK | Baseado originalmente no Zashterminal de Leonardo Berbert"
    WEBSITE = "https://github.com/VaGNaroK/OnyxSH"
    ISSUE_URL = "https://github.com/VaGNaroK/OnyxSH/issues"
...
class ConfigPaths:
    ...
    def __init__(self):
        self.logger = get_logger("onyxsh.config.paths") if UTILS_AVAILABLE else None
        self._setup_paths()
    ...
    def _setup_paths(self):
        try:
            ...
            # Auto-migrate from previous zashterminal directory if exists
            if not self.SETTINGS_FILE.exists() and not self.SESSIONS_FILE.exists():
                legacy_sources = [
                    Path.home() / ".config" / "zashterminal",
                    Path.home()
                    / ".var"
                    / "app"
                    / "org.leoberbert.zashterminal"
                    / "config"
                    / "zashterminal",
                ]
                for legacy_dir in legacy_sources:
                    if legacy_dir.exists():
                        try:
                            ...
                            if self.logger:
                                self.logger.info(
                                    f"Successfully migrated legacy config from {legacy_dir} to {self.CONFIG_DIR}"
                                )
                            break
                        except Exception as e:
                            if self.logger:
                                self.logger.warning(
                                    f"Error migrating legacy config from {legacy_dir}: {e}"
                                )

            # If running in sandbox (e.g. Flatpak) and no settings file exists yet,
            # import existing settings from ~/.config/onyxsh if accessible
            if not self.SETTINGS_FILE.exists():
                host_config = Path.home() / ".config" / "onyxsh"
                if (
                    host_config.exists()
                    and host_config.resolve() != self.CONFIG_DIR.resolve()
                ):
                    import shutil
                    ...
                    for fname in [
                        ...
                        if (
                            host_file.exists()
                            and not (self.CONFIG_DIR / fname).exists()
                        ):
                            try:
    ...
    def _get_legacy_config_dir(self) -> Path:
        ...
        return Path.home() / ".config" / "onyxsh"

    def _get_cache_directory(self) -> Path:
        if xdg_cache := os.environ.get("XDG_CACHE_HOME"):
            return Path(xdg_cache) / "onyxsh"
        return Path.home() / ".cache" / "onyxsh"

    def _use_fallback_paths(self):
        home = Path.home()
        self.CONFIG_DIR = home / ".config" / "onyxsh"
        self.SESSIONS_FILE = self.CONFIG_DIR / "sessions.json"
        self.SETTINGS_FILE = self.CONFIG_DIR / "settings.json"
        self.STATE_FILE = self.CONFIG_DIR / "session_state.json"
        self.LAYOUT_DIR = self.CONFIG_DIR / "layouts"
        self.CACHE_DIR = home / ".cache" / "onyxsh"
        self.LOG_DIR = self.CONFIG_DIR / "logs"
...
class DefaultSettings:
    ...
    def get_defaults() -> Dict[str, Any]:
        return {
            ...
            # Icon Theme Strategy: "onyxsh" (bundled) or "system"
            # Using OnyxSH Icons by default speeds up GTK4 startup
            "icon_theme_strategy": "onyxsh",
            # Shortcuts
            "shortcuts": {
...
def initialize_configuration():
    ...
    logger = get_logger("onyxsh.config") if UTILS_AVAILABLE else None
    try:
        if logger:
            logger.info(
...
try:
    ...
    CONFIG_DIR = os.path.expanduser("~/.config/onyxsh")
