...
class OnyxSHError(Exception):
    """Base exception class for all OnyxSH errors."""
...
class TerminalError(OnyxSHError):
...
class SessionError(OnyxSHError):
...
class StorageError(OnyxSHError):
...
class ValidationError(OnyxSHError):
...
class OnyxSHPermissionError(OnyxSHError):
...
# Alias for backwards compatibility - use OnyxSHPermissionError in new code
PermissionError = OnyxSHPermissionError


class FilePermissionError(OnyxSHPermissionError):
...
class DirectoryPermissionError(OnyxSHPermissionError):
...
def handle_exception(
    ...
    reraise: bool = False,
) -> Optional[OnyxSHError]:
    """Handle an exception by logging it and optionally converting to OnyxSHError."""
    from .logger import log_error_with_context

    log_error_with_context(exception, context, logger_name)
    converted_exception = (
        exception
        if isinstance(exception, OnyxSHError)
        else OnyxSHError(
