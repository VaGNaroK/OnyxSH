...
from .exceptions import OnyxSHError
from .logger import get_logger

# Schema to identify OnyxSH passwords in the system keyring.
SECRET_SCHEMA = Secret.Schema.new(
    "io.github.vagnarok.OnyxSH.Password",
...
def encrypt_session_password(password: str, config_passphrase: str = "") -> str:
    ...
    if len(plain_bytes) > 0xFFFFFFFF:
        raise OnyxSHError("Password is too long for SecureCRT V2 format.")
...
def decrypt_session_password(value: str, config_passphrase: str = "") -> str:
    ...
    try:
        ...
        raise OnyxSHError("Invalid encrypted password format.") from e

    block_size = aes.block_size
    if len(ciphered_bytes) == 0 or len(ciphered_bytes) % block_size != 0:
        raise OnyxSHError("Invalid encrypted password length.")
    ...
    if len(plain_bytes) != plain_len or len(digest) != sha256.digest_size:
        raise OnyxSHError("Invalid encrypted password payload.")
    if sha256.new(plain_bytes).digest() != digest:
        raise OnyxSHError("Encrypted password integrity check failed.")

    try:
        return plain_bytes.decode("utf-8")
    except UnicodeDecodeError as e:
        raise OnyxSHError("Invalid decrypted password encoding.") from e
...
def store_password(session_name: str, password: str) -> bool:
    ...
    if not is_encryption_available():
        raise OnyxSHError("Secret Service API is not available.")
    ...
    try:
        ...
        Secret.password_store_sync(
            ...
            f"Password for OnyxSH session '{session_name}'",
            password,
            None,
        )
        ...
        raise OnyxSHError(f"Failed to store password: {e}") from e
...
def lookup_password(session_name: str) -> Optional[str]:
    ...
    try:
        ...
        raise OnyxSHError(f"Failed to lookup password: {e}") from e
...
def export_all_passwords(sessions_store: Gio.ListStore) -> Dict[str, str]:
    ...
    logger = get_logger("onyxsh.crypto")
    passwords = {}
    if not is_encryption_available():
