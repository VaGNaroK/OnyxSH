...
OSC7_HOST_DETECTION_SNIPPET = (
    'if [ -z "$ONYXSH_OSC7_HOST" ]; then '
    "if command -v hostname >/dev/null 2>&1; then "
    'ONYXSH_OSC7_HOST="$(hostname)"; '
    'elif [ -n "$HOSTNAME" ]; then '
    'ONYXSH_OSC7_HOST="$HOSTNAME"; '
    "elif command -v uname >/dev/null 2>&1; then "
    'ONYXSH_OSC7_HOST="$(uname -n)"; '
    "else "
    'ONYXSH_OSC7_HOST="unknown"; '
    "fi; "
    "fi;"
)
...
class OSC7Parser:
    ...
    def __init__(self):
        ...
        self.logger = get_logger("onyxsh.utils.osc7")
