#!/usr/bin/env python3
"""
Zashterminal Admin Helper (Polkit Backend).
...
logging.basicConfig(
    level=logging.INFO,
    format="[%(asctime)s] [zashterminal-admin-helper] [%(levelname)s] %(message)s",
    handlers=[
        logging.StreamHandler(sys.stderr),
    ],
)
logger = logging.getLogger("zashterminal-admin-helper")
...
def _find_admin_actions_file() -> Path:
    ...
    candidates = [
        ...
        Path("/usr/local/share/zashterminal/data/policies/admin_actions.json"),
        Path("/etc/zashterminal/policies/admin_actions.json"),
    ]
    for c in candidates:
        if c.exists():
            return c
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Zashterminal Admin Helper — Executa ações administrativas autorizadas via Polkit."
    )
