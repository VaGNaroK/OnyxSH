...
"""
OnyxSH - Nautilus Extension
Adds context menu options to open OnyxSH. Supports local paths,
...
try:
    ...
    print(f"OnyxSH Extension: Could not set up localization: {e}")

_ = gettext.gettext

# --- Constants ---
TERMINAL_EXECUTABLE = shutil.which("onyxsh")
REMOTE_URI_SCHEMES = {"sftp", "ssh"}


class OnyxSHExtension(GObject.GObject, Nautilus.MenuProvider):
    """
    Provides context menu items for opening directories in OnyxSH.
    """
    ...
    def _launch(self, command: list[str]):
        ...
        if not TERMINAL_EXECUTABLE:
            print("OnyxSH Error: 'onyxsh' executable not found.")
            return
        try:
    ...
    def _launch_local_session(self, menu_item: Nautilus.MenuItem, files: list[Nautilus.FileInfo]):
        """
        Opens OnyxSH in the specified local or GVFS directory path.
        """
        file = files[0]
        local_path = self._get_local_path(file)
        if not local_path:
            print(f"OnyxSH Error: Could not get local path for {file.get_uri()}")
            return
    ...
    def _launch_remote_ssh_session(self, menu_item: Nautilus.MenuItem, files: list[Nautilus.FileInfo]):
        """
        Parses a remote URI and launches OnyxSH with a direct SSH connection.
        """
        file = files[0]
        uri = file.get_uri()
        try:
            ...
            self._launch(cmd)
            print(f"OnyxSH: Launched SSH session to {target}")
    ...
    def _get_menu_items(self, files: list[Nautilus.FileInfo]) -> list[Nautilus.MenuItem]:
        ...
        if is_remote:
            ...
            ssh_item = Nautilus.MenuItem(
                name="OnyxSH::OpenRemoteSSH",
                label=_("Open in OnyxSH (SSH)"),
            ...
            if local_path:
                gvfs_item = Nautilus.MenuItem(
                    name="OnyxSH::OpenGVFS",
                    label=_("Open in OnyxSH"),
        ...
        # Case 2: Local location (file://)
            ...
            local_item = Nautilus.MenuItem(
                name="OnyxSH::OpenLocal",
                label=_("Open in OnyxSH"),
                tip=_("Open {} in OnyxSH").format(file.get_name()),
            )
