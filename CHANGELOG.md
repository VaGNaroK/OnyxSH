...
- **Integração Global da Exportação de Terminal**: Atalho dedicado `Ctrl + Shift + S`, integração no Menu Principal (Hambúrguer), no Menu de Contexto do Terminal (botão direito), na Command Palette (`Ctrl + Shift + P`) e na barra de busca.
